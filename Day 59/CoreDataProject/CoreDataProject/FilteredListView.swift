...
struct FilteredListView<T: NSManagedObject, Content: View>: View {
    ...
    init(filterKey: String, filterValue: String, sortDescriptors: [NSSortDescriptor], predicateFormat: PredicateFormat, @ViewBuilder content: @escaping (T) -> Content) {
        fetchRequest = FetchRequest<T>(entity: T.entity(), sortDescriptors: sortDescriptors, predicate: NSPredicate(format: predicateFormat.rawValue, filterKey, filterValue))
